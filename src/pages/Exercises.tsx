...
export default function Exercises() {
  ...
  const savePersonalMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingPersonal) {
        await updateExercise(editingPersonal.id, form);
      } else {
    ...
    onSuccess: () => {
      ...
      toast.success(editingPersonal ? 'Ejercicio actualizado' : 'Ejercicio creado');
      setDialogOpen(false);
    },
    onError: (e) => toast.error(e.message),
  });

  // Predefined exercise mutations
  const savePredefinedMutation = useMutation({
    mutationFn: async () => {
      if (!form.name.trim()) throw new Error('Nombre requerido');
      if (form.primary_muscle_ids.length === 0) throw new Error('Al menos un músculo primario requerido');
      if (editingPredefined) {
        await updatePredefinedExercise(editingPredefined.id, form);
      } else {
        await createPredefinedExercise(form);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['predefined_exercises'] });
      toast.success(editingPredefined ? 'Ejercicio actualizado' : 'Ejercicio predefinido creado');
      setDialogOpen(false);
    },
    onError: (e) => toast.error(e.message),
  });
  ...
  const removePersonalMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['exercises'] }); toast.success('Ejercicio eliminado'); },
  });

  const removePredefinedMutation = useMutation({
    mutationFn: deletePredefinedExercise,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['predefined_exercises'] }); toast.success('Ejercicio predefinido eliminado'); },
  });

  function openCreatePersonal() {
    setEditingPersonal(null); setEditingPredefined(null); setIsPredefinedMode(false);
    setForm({ name: '', tracking_type: 'weight_reps', primary_muscle_ids: [], secondary_muscle_ids: [] });
    setDialogOpen(true);
  }

  function openCreatePredefined() {
    setEditingPersonal(null); setEditingPredefined(null); setIsPredefinedMode(true);
    setForm({ name: '', tracking_type: 'weight_reps', primary_muscle_ids: [], secondary_muscle_ids: [] });
    setDialogOpen(true);
  }

  function openEditPersonal(ex: Exercise) {
    setEditingPersonal(ex); setEditingPredefined(null); setIsPredefinedMode(false);
  ...
  function openEditPredefined(ex: PredefinedExercise) {
    setEditingPredefined(ex); setEditingPersonal(null); setIsPredefinedMode(true);
    setForm({ name: ex.name, tracking_type: ex.tracking_type, primary_muscle_ids: ex.primary_muscle_ids ?? [], secondary_muscle_ids: ex.secondary_muscle_ids ?? [] });
    setDialogOpen(true);
  }

  function handleSave() {
    if (isPredefinedMode) savePredefinedMutation.mutate();
    else savePersonalMutation.mutate();
  }

  function renderExerciseRow(ex: { id: string; name: string; tracking_type: string; primary_muscle_ids: number[] | null; secondary_muscle_ids: number[] | null }, isPredefined: boolean) {
    return (
      <div key={ex.id} className="flex items-center justify-between p-3 rounded-lg bg-card border">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="font-medium text-sm">{ex.name}</p>
            {isPredefined && <Badge variant="secondary" className="text-[10px] px-1.5 py-0">Predefinido</Badge>}
          </div>
    ...
    <div className="p-4 pb-20 max-w-lg mx-auto">
      ...
      <Tabs defaultValue="predefined">
        <TabsList className="w-full">
          <TabsTrigger value="predefined" className="flex-1">Predefinidos ({filteredPredefined.length})</TabsTrigger>
          <TabsTrigger value="personal" className="flex-1">Mis ejercicios ({filteredPersonal.length})</TabsTrigger>
        </TabsList>

        <TabsContent value="predefined">
          {admin && (
            <div className="mb-3">
              <Button size="sm" onClick={openCreatePredefined}><Plus className="h-4 w-4 mr-1" />Nuevo predefinido</Button>
            </div>
          )}
          <div className="space-y-2">
            {filteredPredefined.map(ex => renderExerciseRow(ex, true))}
            {filteredPredefined.length === 0 && <p className="text-center text-muted-foreground text-sm py-8">No hay ejercicios predefinidos</p>}
          </div>
        </TabsContent>

        <TabsContent value="personal">
          <div className="mb-3">
            <Button size="sm" onClick={openCreatePersonal}><Plus className="h-4 w-4 mr-1" />Nuevo</Button>
          </div>
          <div className="space-y-2">
            {filteredPersonal.map(ex => renderExerciseRow(ex, false))}
            {filteredPersonal.length === 0 && <p className="text-center text-muted-foreground text-sm py-8">No hay ejercicios personales</p>}
          </div>
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {isPredefinedMode
                ? (editingPredefined ? 'Editar predefinido' : 'Nuevo predefinido')
                : (editingPersonal ? 'Editar ejercicio' : 'Nuevo ejercicio')}
            </DialogTitle>
          </DialogHeader>
